...
function sqlStmts(db: TypedSqlStorage) {
	return {
		getChanges: db.exec<{ totalChanges: number; lastRowId: number }>(
...
export class D1DatabaseObject extends MiniflareDurableObject {
	...
	#changes() {
		const changes = (this.#stmts.getChanges as any).toArray();
		return changes!;
	}
	...
	#query = (format: D1ResultsFormat, query: D1Query): D1SuccessResponse => {
		...
		const cursor = this.db.exec(query.sql, ...params);
	...
	@POST("/query")
	@POST("/execute")
	async queryExecute(req: Request): Promise<Response> {
