...
// Use TextEncoder/TextDecoder for buffer operations in Workers
...
function generateBlobId(): BlobId {
	const idBuffer = new ArrayBuffer(40);
	const view = new Uint8Array(idBuffer);
	crypto.getRandomValues(view.subarray(0, 32));
	
	// Write timestamp as big-endian 64-bit integer
	const timestampView = new DataView(idBuffer);
	timestampView.setBigUint64(32, BigInt(performance.timeOrigin + performance.now()), false);
